...
void main() {
  ...
  final notification = AppNotification(
    ...
    display: const NotificationDisplay(title: 'Invitation', body: 'Open ChillGo'),
    createdAt: createdAt,
  ...
  test('notification is unread and expires exactly at its boundary', () {
    ...
    expect(notification.isExpiredAt(notification.expiresAt.subtract(const Duration(microseconds: 1))), isFalse);
  });
  ...
  test('page identifies a following cursor', () {
    ...
    expect(NotificationPage(items: [notification], nextCursor: cursor).hasMore, isTrue);
    expect(const NotificationPage(items: []).hasMore, isFalse);
  });

  test('unavailable results never expose a target', () {
    const result = NotificationOpenResult.unavailable(NotificationUnavailableReason.expired);
