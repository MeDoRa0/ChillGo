...
- **Client access**: exact recipient-only, source-aware gets; direct lists and all writes/deletes denied. Center pages use the authenticated, App Check-protected `notificationCenterPage` callable.
...
Rules re-evaluate source availability and current recipient authorization on every exact get. Crew invitations require a pending target invitation. Outing and agreement records require current source, crew membership, and participation. Arrival additionally requires Meeting, Accepted attendance, and current live-meetup eligibility. Direct lists are denied; the trusted center callable applies the same reauthorization to every candidate before returning it. Failed access checks deny immediately; transition cleanup removes the record physically.
...
Clients never read registrations or tokens. Supported registrations are Android/iOS only. Token refresh, invalid-provider response, sign-out, and 30-day inactivity unregister a target.
...
The trusted center callable scans `recipientUserId == currentUser`, `createdAt desc`, document ID desc in bounded chunks and returns authorized pages of at most 50 records with an opaque cursor. Direct client list queries are denied. Indexes cover center scans and bounded server cleanup by expiry, source IDs, outing, crew, and recipient. TTL overrides apply to record expiry and short-lived command/event/work/device fields; scheduled cleanup, not TTL, provides the product boundary.
