...
class FirestoreLiveMeetupClock implements TrustedClock {
  ...
  DateTime get now {
    ...
    if (offset == null) throw const LiveMeetupOfflineFailure();
  ...
  Future<DateTime> _createProbe(String uid) async {
    final collection = firestore.collection('live_meetup_time_probes');
    ...
    try {
      ...
      final timestamp = readFirestoreTimestamp(snapshot.data()?['requestedAt']);
      if (timestamp == null) throw const LiveMeetupOfflineFailure();
      return timestamp;
    } finally {
  ...
  Future<void> _removeProbe(DocumentReference<Map<String, dynamic>> ref) async {
    try {
      ...
      // Scheduled repair removes abandoned owner-private probes.
    } finally {
      _probes.remove(ref);
    }
  }
